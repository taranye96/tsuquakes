...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  3 14:38:06 2020

@author: tnye
"""

###############################################################################
# Script that plots the observed acceleration spectra and compares Kappa to the
...
# Gather files 
acc_files = np.sort(np.array(glob(acc_dir + '*HNE*.mseed')))
...
for s in sortby:
    ...
    fig.delaxes(axs[5][1])
    fig.delaxes(axs[5][2])
    fig.suptitle('Observed Spectra', fontsize=12, y=1)
    plt.subplots_adjust(left=0.1, bottom=0.05, right=0.9, top=0.925, wspace=0.1, hspace=0.35)
    
    # Save figure
    plt.savefig(f'/Users/tnye/tsuquakes/plots/kappa/sortby_{s[1]}_obs.png', dpi=300)
    plt.close()
...
# Save figure
plt.savefig('/Users/tnye/tsuquakes/plots/kappa/kappa_slopes_obs.png', dpi=300)
plt.close()
