...
gnss_stations = np.array(pd.read_csv('/Users/tnye/tsuquakes/data/stations/disp_stations.csv'))
...
wf_dir = '/Users/tnye/FakeQuakes/8min_16cores/sm/run.' + run + '/'

# Define path to save waveforms
disp_figpath = '/Users/tnye/FakeQuakes/MentawaiSM2/plots/waveforms/disp/run.' + run + '/'
sm_figpath = '/Users/tnye/FakeQuakes/MentawaiSM2/plots/waveforms/sm/run.' + run + '/'
vel_figpath = '/Users/tnye/FakeQuakes/velocity_8_16/run.' + run + '/'

# Get disp data
disp_files = np.array(sorted((glob(wf_dir + '/*.L*.sac'))))
sm_files = np.array(sorted((glob(wf_dir + '/*.H*.sac'))))
...
for st in streams:
    ...
    # Calc hypdist
    st_index = np.where(gnss_stations==tr1.stats.station)[0][0]
    ...
    # Save figure
    figpath2 = disp_figpath + station + '.wf.png'
...
streams = []
stream_counter = []
for i, file in enumerate(sm_files):
    st = read(file)
    tr = st[0]
    ...
    if i == 0:
        ...
        if tr.stats.station == read(sm_files[i-1])[0].stats.station:
            stream_counter[0] = stream_counter[0] + st
        else:
            streams.append(stream_counter)
            stream_counter = []
...
for st in streams:
    ...
    # Calc hypdist
    st_index = np.where(st==tr1.stats.station)[0][0]
    st_lon = sm_stations[st_index][1]
    st_lat = sm_stations[st_index][2]
    epdist = geopy.distance.geodesic((epi_lat, epi_lon), (st_lat, st_lon)).km
    hypdist = np.sqrt(hypdepth**2 + epdist**2)
    ...
    # Save figure
    figpath2 = sm_figpath + station + '.wf.png'
    plt.savefig(figpath2, bbox_inches='tight', dpi=300)
    plt.close()