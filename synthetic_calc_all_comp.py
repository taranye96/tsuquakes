...
for run in runs:
    ...
    flatfile_path = f'/Users/tnye/tsuquakes/flatfiles/spec_test/{project}/{run}.csv'
    ...
    for data in data_types:
        ...
        for i, station in enumerate(stn_name_list):
            ...
            for channel in channel_list[i]:
                instruments.append(channel[0:2])    
            ...
            #     a new dataframe and reset the index so it starts at 0
                ...
                if (len(instruments) <= 3) & (len(np.unique(instruments))) == 1:
                    ...
                    print('Event %s, Station %s' % (eventname,station))
                ...
                if data == 'disp':
                    ...
                    ## Disp Spectra
                    E_spec_data = IM_fns.calc_spectra(E_record, data)
                    N_spec_data = IM_fns.calc_spectra(N_record, data)
                    Z_spec_data = IM_fns.calc_spectra(Z_record, data)
                    ...
                    # Plot spectra
                    IM_fns.plot_spectra(E_record, data, 'E', project, run)
                    IM_fns.plot_spectra(N_record, data, 'N', project, run)
                    IM_fns.plot_spectra(Z_record, data, 'Z', project, run)
                ...
                if data == 'sm':
                    ...
                    pga_E = tmf.get_peak_value(E_record)
                    pga_N = tmf.get_peak_value(N_record)
                    ...
                    # Plot spectra
                    IM_fns.plot_spectra(E_record, 'acc', 'E', project, run)
                    IM_fns.plot_spectra(N_record, 'acc', 'N', project, run)
                    IM_fns.plot_spectra(Z_record, 'acc', 'Z', project, run)
                    ...
                    ## Vel Spectra
                    E_spec_vel = IM_fns.calc_spectra(E_vel, data)
                    N_spec_vel = IM_fns.calc_spectra(N_vel, data)
                    Z_spec_vel = IM_fns.calc_spectra(Z_vel, data)
                    # Combine into one array and append to main list
                    vel_spec = np.concatenate([E_spec_data,N_spec_data,Z_spec_data])
                    vel_speclist.append(vel_spec.tolist())
                    # Plot spectra
                    IM_fns.plot_spectra(E_vel, 'vel', 'E', project, run)
                    IM_fns.plot_spectra(N_vel, 'vel', 'N', project, run)
                    IM_fns.plot_spectra(Z_vel, 'vel', 'Z', project, run)
    ...
    ####### NEED to fix things with spectra
