...
for run in runs:
    ...
    for data in data_types:
        ...
        for i, station in enumerate(stn_name_list):
            ...
            if data == 'sm':
               # Convert acceleration record to velocity 
               E_vel_unfilt = tmf.accel_to_veloc(E_filt)
               N_vel_unfilt = tmf.accel_to_veloc(N_filt)
               Z_vel_unfilt = tmf.accel_to_veloc(Z_filt)
            ...
            if data == 'disp':
                ...
                disp_euc_norm = avg.get_eucl_norm_3comp(E_record[0].data,
                                                   N_record[0].data,
                                                   Z_record[0].data)
                # Calculate PGD
                pgd = np.max(np.abs(disp_euc_norm))
    ...
    dataset_dict = {'eventname':eventnames,'country':countries, 'origintime':origintimes,
                    ...
                    'duration_3comp':comp3_Td_list, 'pgd':pgd_list, 'pga':pga_list,
                    'pgv':pgv_list,'tPGD_origin':tPGD_orig_list,
