...
# FakeQuakes, calculates IMs and spectra, and stores it all in a flatfile.
...
obs_wf_dir = '/Users/tnye/tsuquakes/data/processed_waveforms/average/flatfiles'
...
eq_table_path = f'{data_dir}/events.csv'   
        ...
        for data in data_types:
            ...
            for i, group in enumerate(stn_files):
                ...
                if stn in station_names:
                    ...
                    # Add noise to GNSS
                    if data == 'gnss':
                        st_E_noisy,st_N_noisy,st_Z_noisy = tmf.add_real_gnss_noise(E_raw,N_raw,Z_raw)
                    ...
                    if data == 'sm':
                        ...
                        wf_amps = tmf.compute_rotd50(E_record[0].data,N_record[0].data)
                    ...
                    if data == 'gnss':
                        ...
                        # Calculate PGD
                        eucnorm = tmf.get_eucl_norm_3comp(tr_E_short.data, tr_N_short.data, tr_Z_short.data)
                        ...
                        tPGD = tmf.calc_time_to_peak(pgd, tr_E_short,np.abs(eucnorm),origin,hypdist)
                        tPGD_list = np.append(tPGD_list,tPGD)
            
                        # Disp Spectra
                        bins, E_spec_data = tmf.calc_spectra(st_E_noisyfilt, data)
                        bins, N_spec_data = tmf.calc_spectra(st_N_noisyfilt, data)
                        bins, Z_spec_data = tmf.calc_spectra(st_Z_noisyfilt, data)
                        
                        bins_trim, E_spec_data_trim = tmf.calc_spectra(Stream(tr_E_short), data)
                        bins_trim, N_spec_data_trim = tmf.calc_spectra(Stream(tr_N_short), data)
                        bins_trim, Z_spec_data_trim = tmf.calc_spectra(Stream(tr_Z_short), data)
                    ...
                    if data == 'sm':
                        ...
                        tPGA = tmf.calc_time_to_peak(pga, E_record[0],np.abs(wf_amps),origin,hypdist)
                        tPGA_list = np.append(tPGA_list,tPGA)
            
                        # Acc Spectra
                        bins, E_spec_data = tmf.calc_spectra(E_record, data)                
                        bins, N_spec_data = tmf.calc_spectra(N_record, data)
                        bins, Z_spec_data = tmf.calc_spectra(Z_record, data)
            ...
            if data == 'gnss':
                ...
                ## Displacement
                tmf.plot_spec_comp(plot_dir,syn_freqs,syn_spec,disp_obs_freqs,disp_obs_spec,stn_list,hypdists,'disp',home,parameter,project,run,'full')
                # tmf.plot_spec_comp(plot_dir,syn_freqs_trim,syn_spec_trim,disp_obs_freqs_trim,disp_obs_spec_trim,stn_list,hypdists,'disp',home,parameter,project,run,'trim')
                
                tmf.plot_wf_comp(plot_dir,syn_times,syn_amps_avg,stn_list,hypdists,'disp',wf_type,home,parameter,project,run,'avg',E_record[0].times('UTCDateTime')[0],UTCDateTime("2010-10-25T14:47:02"))
            
            if data == 'sm':
                
                ## Acceleration 
                tmf.plot_spec_comp(plot_dir,syn_freqs,syn_spec,acc_obs_freqs,acc_obs_spec,stn_list,hypdists,'acc',home,parameter,project,run,'full')
                
                tmf.plot_wf_comp(plot_dir,syn_times,syn_amps_avg,stn_list,hypdists,'acc',wf_type,home,parameter,project,run,'avg',E_record[0].times('UTCDateTime')[0],E_record[0].times('UTCDateTime')[-1])
