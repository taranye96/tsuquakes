...
# The functions use IM and spectra values calculated using 0_calc_obs_IMs.py for
# observed data and parallel_calc_syn_IMs.py for synthetic data. These functions
...
def calc_res(obs_file, home_dir, parameter, project, run, dtype):
    ...
    if dtype=='gnss':
        ...
        syn_spectra = np.delete(np.array(syn_df.iloc[:,23:34]),1,axis=0)
        
        # Observed values
        obs_pgd = np.array(obs_df['pgd'])
        obs_tPGD = np.array(obs_df['tPGD_origin'])
        obs_spectra = np.array(obs_df.iloc[:,23:34]) 
        ...
        syn_spectra = np.array(syn_df.iloc[:,23:34]) # syn_df only has 1 tPGD column whereas obs_df has 2
