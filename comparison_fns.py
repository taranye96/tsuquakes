...
def plot_spec_comp(syn_freqs, syn_amps, obs_freqs, obs_amps, stn_list, hypdists, data_type, home, parameter, project, run):
    """
    ...
    if data_type == 'disp':
        ...
        fig.text(0.45, 0.075, (r'$\bf{' + 'DataType:' '}$' + '' + data_type))
        plt.tight_layout()
        
        if not path.exists(f'{home}/{parameter}/{project}/plots/comparison/spectra'):
            makedirs(f'{home}/{parameter}/{project}/plots/comparison/spectra')
            
        plt.savefig(f'{home}/{parameter}/{project}/plots/comparison/spectra/{run}_{data_type}.png', dpi=300)
        plt.close()
        ...
        fig.text(0.45, 0.065, (r'$\bf{' + 'DataType:' '}$' + '' + data_type))
        plt.tight_layout()
...
def plot_wf_comp(syn_times, syn_amps, obs_times, obs_amps, stn_list, hypdists, data_type, home, parameter, project, run):
    ...
    if data_type == 'disp':
        ...
        fig.text(0.385, 0.09, (r'$\bf{' + 'DataType:' '}$' + '' + data_type))
        plt.subplots_adjust(left=0.1, right=0.9, bottom=0.075, top=0.925,
                            wspace=0.3, hspace=0.4)
    
        if not path.exists(f'{home}/{parameter}/{project}/plots/comparison/wf/'):
          makedirs(f'{home}/{parameter}/{project}/plots/comparison/wf')
      
        plt.savefig(f'{home}/{parameter}/{project}/plots/comparison/wf/{run}_{data_type}.png', dpi=300)
        plt.close()
